...
function buildEmailHTML(formData, results) {
  const { name, companyName, email, companyWebsite, jobTitle, targetMarket, industry, acv, salesCycle, quarterlyRevenueTarget, mqlsPerMonth, mqlToSql, sqlToCustomer, currentMeetings, marketingSpend, outboundType, databaseSize, databaseAccuracy, crm } = formData
  const { expectedMeetings, pipelineQuarter, revenueMonthly, gtmScore, gtmLabel, plan, meetingGap, pipelineGap, revenueGap, currentMeetingsFunnel, requiredMeetings, industry: ind } = results
  ...
  return `
    ...
    <body style="font-family: Arial, sans-serif; font-size: 14px; color: #000000; background: #ffffff; margin: 0; padding: 20px;">
      ...
      <table cellpadding="6" cellspacing="0" border="1" style="border-collapse: collapse; width: 100%; max-width: 560px;">
        ...
        <tr><td>Industry Benchmark Applied</td><td>${industry}</td></tr>
